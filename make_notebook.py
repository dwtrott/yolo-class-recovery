...
code("""#@title 2. Build the testbed: fine-tune YOLOv8n on 4 non-COCO classes, then strip the labels  (≈5 min on T4)
#@markdown The stripped copy is `testbed/mystery.pt` — that's what the recovery pipeline sees.
#@markdown `testbed/truth.json` keeps the real names for scoring only.
from classrecovery.testbed import build_testbed
EPOCHS = 15  #@param {type:"integer"}
paths = build_testbed("testbed", dataset="african-wildlife.yaml", base="yolov8n.pt", epochs=EPOCHS, imgsz=640)
print(paths)
import json; print("ground truth (don't peek if you want to play fair):", json.load(open("testbed/truth.json"))["names"])""")

code("""#@title 3. Weight diff against the base checkpoint  (seconds)
...
code("""#@title 4. Prompt search over the built-in vocabulary  (≈4 min on T4 with sd-turbo)
...
code("""#@title 5. Guided recovery + CLIP naming for every class  (≈1 min / class on T4)
...
code("""#@title 6. Launch the GUI  (prints a public gradio.live link; also renders inline)
